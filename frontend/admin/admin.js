...
async function updateAdminCredentials() {
    ...
    try {
        await fetchProtected(`${API_URL}/admin/credentials`, {
            method: 'POST',
            body: JSON.stringify({ 
                current_password, 
                new_username, 
...
async function loadRequests() {
    try {
        ...
        renderRequestsPage(currentReqPage);
    } catch(e) { console.error("Помилка завантаження заявок", e); }
}

function getStatusColor(status) {
    if (status === 'В роботі') return '#f59e0b';
    if (status === 'Завершено') return '#10b981';
    if (status === 'Відмова') return '#ef4444';
    return '#0ea5e9';
}
            ...
            <div class="req-card">
                <div class="req-info">
                    ...
                    <div style="margin-top: 8px;">
                        <span style="font-size: 15px; font-weight: 500;">📞 <a href="tel:${safePhone}" style="color: #0f172a; text-decoration: none;">${safePhone}</a></span>
                    </div>
                </div>
...
async function loadTeam() {
    ...
    } catch(e) { console.error("Помилка завантаження команди", e); }
}
...
async function deleteTeamMember(id) {
    ...
    try {
        ...
        loadTeam();
    } catch(e) { showStatus('Помилка видалення спеціаліста', true); }
}
...
document.getElementById('team-form').addEventListener('submit', async (e) => {
    ...
    if (file) {
        try {
            ...
            showStatus('Не вдалося завантажити фото', true);
            return;
        }
    }
    ...
    try {
        ...
        showStatus('Дані по команді збережено!');
        loadTeam();
    } catch(e) { showStatus('Помилка збереження спеціаліста', true); }
});
